...
class TerminalStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, vpc: ec2.Vpc, security_group: ec2.SecurityGroup,
        ...
        ubuntu_bootstrapping.add_commands(
            'apt -y update',
            'apt -y upgrade',
            'apt -y install python3 python3-pip unzip',
        ...
        apache_handle = ec2.InitServiceRestartHandle()
        ...
        init_ubuntu = ec2.CloudFormationInit.from_config_sets(
            ...
            configs={
                'apache': ec2.InitConfig([
                    ec2.InitPackage.apt(
                        package_name='apache2',
                        service_restart_handles=[apache_handle],
                    ),
                    ec2.InitFile.from_string("/var/www/html/index.php", "<?php\nphpinfo();\n?>",
                                             mode="000644",
                                             owner="www-data",
                                             group="www-data",
                                             service_restart_handles=[apache_handle],
                    ...
                    ec2.InitService.enable("apache2",
                                           enabled=True,
                                           ensure_running=True,
                                           service_restart_handle=apache_handle
                                           )
                ]),
                # 'ssl': ec2.InitConfig([
                #     ec2.InitCommand.shell_command("a2ensite default-ssl"),
                # ])
            }
        )
